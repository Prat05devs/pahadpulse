 ...
 *
 * Called nightly by the in-process scheduler and by `npm run db:rollup`. It borrows one
 * client from the shared pool and never ends the pool — that is the caller's business.
 */
...
export async function rollupObservations(): Promise<Result<RollupReport, RequestError>> {
  let client;
  try {
    client = await db.connect();
  } catch (error) {
    logger.error('rollup could not get a connection', { error: describeError(error) });
  ...
  try {
     ...
     *
     * A timestamp cutoff lands mid-day and splits the boundary day in two. The first night
     * summarised that day's morning and deleted it; the next night the upsert below rebuilt
     * the same day from only the afternoon that was left, silently overwriting the morning's
     * min, max, mean and sample count. Every rolled-up day lost part of itself, in the table
    ...
    if (cutoff === undefined) {
      ...
      logger.error('rollup cutoff query returned no row');
      return err(ERRORS.DATABASE_ERROR);
    }
    ...
    logger.info('rollup complete', { retentionDays: RAW_RETENTION_DAYS, ...report });
    return ok(report);
  } catch (error) {
    await client.query('ROLLBACK').catch(() => undefined);
    logger.error('rollup failed', { error: describeError(error) });
    return err(ERRORS.DATABASE_ERROR);
  } finally {
    client.release();
  }
