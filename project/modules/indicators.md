...
health facilities, economy and industry, connectivity. One shape — _this indicator, for this
area, at this vintage, from this source_ — serves the state overview, every district dashboard,
...
| Forest cover | 63%    | 45.44% | matched neither FSI measure. Forest _cover_ is 45.44%, recorded forest _area_ is ~71%. Verified: 24,303.04 / 53,483 km² = 45.44%.                                                  |
...
therefore _not_ listed as pending: "being compiled" promises a figure will arrive, and that
...
**Response 200** — indicator rows present for _both_ areas, each with both values, both
...
| Web     | `/intelligence`              | Server Component       | evidence catalogue, sector coverage, district benchmarks, and practical tool map |

Every rendered value carries a `<SourceBadge>` and its vintage. A number without both is a
blocking review comment.

The intelligence workspace also shows catalogue entries that do not yet have a value. It
labels full, partial, state-only, catalogue-only, and temporarily unavailable coverage rather
than turning absence into a metric. URL query parameters provide shareable sector, status, and
text filters without adding client state.
...
rules of its own — health facility _locations_, for instance, are points rather than statistics.
...
- [ ] The full v1 indicator catalogue — which keys, in which categories, with which units. Needs
      to be fixed before the seed migration. — _owner:_ `<TBD>`
