...
| Hosting | `<TBD — must be fixed before the IMD whitelisting request>` | |
...
alert *content* is fully public and readable with no account.
...
| Vintage | The date the data *describes*, not the date we fetched it. A 2011 census figure fetched today has a 2011 vintage. Distinct from `fetched_at`. |
...
| `40xxx` | geography |
| `50xxx` | indicators |
| `55xxx` | *reserved* — migration |
| `60xxx` | alerts |
| `70xxx` | hydromet |
| `80xxx` | roads |
| `85xxx` | tourism |
| `90xxx` | datasets (upstream and ingestion failures) |
| `95xxx` | *reserved* — governance |
...
- [ ] Hosting platform and region — blocks the static egress IP, which blocks the IMD
      whitelisting request. Highest-priority unknown on the project. — *owner:* `<TBD>`
- [ ] Default locale: `hi` or `en`? Recommend `hi` for a resident-facing state portal, with
      `/en` available. Affects the root redirect and SEO canonical URLs. — *owner:* `<TBD>`
- [ ] Is there a state-government stakeholder who can shorten the departmental data requests?
      Four of the eight modules depend on letters with no SLA. — *owner:* `<TBD>`
