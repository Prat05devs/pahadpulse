...
export function BudgetAllocationSection({ budget }: BudgetAllocationSectionProps) {
    ...
    return (
      ...
      >
        <div>
          ...
          <h2 id="budget-allocation-heading" className="mt-1 text-xl font-semibold text-text-light">
            Budget explorer
          </h2>
        </div>
        <div className="surface-card p-5" role="alert">
          <p className="text-sm leading-relaxed text-muted-foreground">
            The official budget dataset could not be loaded. The rest of the governance workspace
            remains available while the data service reconnects.
          </p>
        </div>
      </section>
    );
  ...
  const revenueTotal = budget.departments.reduce(
    (sum, item) => sum + item.revenue.voted + item.revenue.charged,
    0
  );
  const capitalTotal = budget.departments.reduce(
    (sum, item) => sum + item.capital.voted + item.capital.charged,
    0
  );
  const chargedTotal = budget.departments.reduce(
    (sum, item) => sum + item.revenue.charged + item.capital.charged,
    0
  );
  const topDepartments = budget.departments.slice(0, 8);
  const topFiveShare = budget.departments
    .slice(0, 5)
    .reduce((sum, department) => sum + department.share, 0);
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm leading-relaxed text-muted-foreground">
            Compare nineteen official budget estimates, then inspect demand-wise provisions where
            the source publishes a compatible table. Allocations are not actual expenditure.
          </p>
        </div>
        ...
        <form action="/governance" className="flex flex-wrap items-end gap-2">
          <label className="grid gap-1 text-xs font-medium text-muted-foreground">
            ...
            <select
              ...
              className="min-h-11 min-w-36 rounded-lg border border-border bg-surface px-3 text-sm font-semibold text-text-light outline-none focus-visible:ring-2 focus-visible:ring-accent"
            >
              {budget.availableYears.map((year) => (
          ...
          <button
            type="submit"
            className="min-h-11 rounded-lg bg-accent px-4 text-sm font-semibold text-white transition-colors hover:bg-accent/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2"
          >
            View year
          </button>
        </form>
      ...
      {summary === null ? (
        <div className="surface-card p-5" role="alert">
          <p className="text-sm text-muted-foreground">
            No official summary is held for {budget.fiscalYear}. Choose one of the archived years.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="surface-card overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-4 sm:px-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p id="budget-history-description" className="text-xs text-muted-foreground">
                    Current-year budget estimate from each annual document; amounts in crore.
                  </p>
                </div>
              </div>
              <a
                href="https://budget.uk.gov.in/budgets/"
                target="_blank"
            ...
            <div className="max-h-[30rem] overflow-auto">
              <table
                ...
                aria-describedby="budget-history-description"
              >
                <caption className="sr-only">
                  Uttarakhand budget estimate history from 2008-09 to 2026-27
                </caption>
                    ...
                    return (
                      <tr key={year.fiscalYear} className={selected ? 'bg-accent/10' : undefined}>
                        <th scope="row" className="px-4 py-3 text-left">
                          <Link
                            href={`/governance?budgetYear=${year.fiscalYear}#budget-allocation-heading`}
                        ...
                        <td className="px-4 py-3 text-right font-mono text-xs tabular-nums text-muted-foreground">
                          {formatCrore(year.revenueExpenditure)}
                        </td>
                        <td className="px-4 py-3 text-right font-mono text-xs tabular-nums text-muted-foreground">
                          {formatCrore(year.capitalExpenditure)}
                        </td>
                        <td className="px-4 py-3 text-right">
                          <span className="font-mono text-xs font-semibold tabular-nums text-text-light">
                            {formatCrore(year.totalExpenditure)}
                          </span>
                          <span
                          ...
                          >
                            <span
                              ...
                              style={{ width: `${(year.totalExpenditure / maximumOutlay) * 100}%` }}
                            />
                          </span>
                        </td>
                        <td className="px-4 py-3 text-right font-mono text-xs tabular-nums text-muted-foreground">
                          {yearChange(
                            year.totalExpenditure,
                            historyAscending[index - 1]?.totalExpenditure
                          )}
                        </td>
                        <td className="px-4 py-3 text-xs text-muted-foreground">
                          {hasDemandTable ? 'Summary + 31 demands' : 'State summary'}
                        </td>
                      </tr>
                    );
                  })}
        ...
        <div className="surface-card flex items-start gap-3 p-5">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
              The {budget.fiscalYear} Budget at a Glance uses an older presentation and does not
              provide the comparable 31-demand table used below for 2018-19 onward. No departmental
            ...
            {summary !== null ? (
              ...
              >
                Open the {budget.fiscalYear} source PDF{' '}
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Top five account for {topFiveShare.toFixed(1)}% of the demand total
                </p>
              </div>
            </div>
              ...
              {topDepartments.map((department) => (
                <li key={department.demandNo}>
                  <div className="flex items-start justify-between gap-3 text-sm">
                    <span className="min-w-0 font-medium leading-snug text-text-light">
                      {department.name}
                    </span>
                    <span className="shrink-0 font-mono text-xs font-semibold tabular-nums text-text-light">
                  ...
                  <p className="mt-1 text-right text-[0.68rem] tabular-nums text-muted-foreground">
                    {department.share.toFixed(2)}% of demand total
                  </p>
                </li>
              ))}
            </ol>
          ...
          <div className="surface-card overflow-hidden">
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border p-4 sm:px-5">
              <div>
                <h3 className="font-semibold text-text-light">
                  All demands for grants · {budget.fiscalYear}
                </h3>
                <p
                  id="budget-table-description"
                  className="mt-1 text-xs leading-relaxed text-muted-foreground"
                >
                  Voted and charged provisions remain separate. Amounts are displayed in crore.
                </p>
              </div>
              {summary !== null ? (
                <a
                ...
                >
                  Source PDF <ExternalLink className="size-4" aria-hidden="true" />
                </a>
              ) : null}
            </div>
            <div className="overflow-x-auto">
              <table
                ...
                aria-describedby="budget-table-description"
              >
                <caption className="sr-only">
                  Uttarakhand department budget estimates for {budget.fiscalYear}
                </caption>
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3 font-mono text-xs tabular-nums text-muted-foreground">
                          {department.demandNo.toString().padStart(2, '0')}
                        </td>
                        ...
                        <td className="px-4 py-3 text-right font-mono text-xs font-semibold tabular-nums text-text-light">
                          {formatCrore(department.total)}
                        </td>
                        <td className="px-4 py-3 text-right font-mono text-xs tabular-nums text-muted-foreground">
                ...
                <tfoot className="border-t border-border bg-muted/40 font-semibold text-text-light">
                  <tr>
                    ...
                    <td className="px-4 py-3 text-right font-mono text-xs tabular-nums">
                      {formatCrore(revenueTotal)}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-xs tabular-nums">
                      {formatCrore(capitalTotal)}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-xs tabular-nums">
                      {formatCrore(chargedTotal)}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-xs tabular-nums">
                      {formatCrore(budget.departmentTotal)}
                    </td>
            ...
            <p className="border-t border-border px-4 py-3 text-xs leading-relaxed text-muted-foreground sm:px-5">
              {summary?.provenance?.attribution ??
                'Source: Budget Directorate, Government of Uttarakhand'}
              . Stored in thousands of rupees as published. Small differences from the headline can
              reflect its two-decimal crore rounding.
            </p>
          </div>
        </div>
      )}
...
function BudgetMetric({ label, value }: { label: string; value: number }) {
  return (
    <div className="surface-card p-4">
      ...
      <p className="mt-1 font-display text-xl font-semibold tabular-nums text-text-light">
        {formatCrore(value)}
      </p>
    </div>
  );
